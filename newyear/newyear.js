...
function functionConfirm(msg) {
    ...
    jQuery('#link_url').val(`${window.location.origin.replace("http:", "https:")}${window.location.pathname}?text=${jQuery('#c-form__input').val()}`);
	  ...
	  jQuery.get(`https://request.rohittechzone.com/timeapiindia`, function(response) {
       ...
       if (navigator.share) {
         navigator.share({
           ...
           url: jQuery('#link_url').val().replace(" ", "%20")
         })
